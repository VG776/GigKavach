...
def _add_origin(origin_set: set[str], origin: str | None) -> None:
    normalized = _normalize_origin(origin)
    if normalized:
        origin_set.add(normalized)


# ─── CORS Middleware ──────────────────────────────────────────────────────────
# Allow the deployed frontend and local developer shells to reach the API.
...
for local_origin in (
    ...
    "http://127.0.0.1:3000",
    "http://127.0.0.1:5173",
    settings.WORKER_PWA_LOCAL_URL,
    "http://frontend:3000",
    "http://frontend:5173",
):
    _add_origin(cors_origins, local_origin)
...
for production_origin in (
    ...
    os.getenv("WORKER_PWA_URL"),
):
    _add_origin(cors_origins, production_origin)

vercel_url = os.getenv("VERCEL_URL")
if vercel_url:
    _add_origin(cors_origins, f"https://{vercel_url}")

for extra_origin in (os.getenv("DEV_SERVER_URL"), os.getenv("CORS_ORIGIN"), os.getenv("CORS_ALLOWED_ORIGIN")):
    _add_origin(cors_origins, extra_origin)

cors_origin_list = sorted(cors_origins)

logger.debug(f"CORS Origins configured: {cors_origin_list}")
