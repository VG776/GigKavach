...
async def get_shared_worker_profile(
    ...
    try:
        ...
        return {
            ...
            "pin_codes": worker.get("pin_codes", []),
            "language": worker.get("language", "en"),
...
async def update_shared_worker_profile(
    ...
    try:
        ...
        if request.upi_id is not None:
            if "@" not in request.upi_id:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="upi_id must contain '@'")
            updates["upi_id"] = request.upi_id.strip().lower()

        if request.pin_codes is not None:
            pin_codes = [str(pin).strip() for pin in request.pin_codes]
            if not (1 <= len(pin_codes) <= 5):
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="pin_codes must contain 1 to 5 values")
            for pin in pin_codes:
                if not pin.isdigit() or len(pin) != 6:
                    raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Invalid pin code: {pin}")
            updates["pin_codes"] = pin_codes
...
@router.post("/session-login/{token}")
async def session_login_with_share_token(
    token: str,
    request: WorkerSessionLoginRequest,
    supabase = Depends(get_supabase),
):
    """
    Login handshake for worker PWA session.
    This is demo auth (password not persisted); only session key is stored in Redis.
    """
    try:
        token_result = (
            supabase.table("share_tokens")
        ...
        if len((request.digilocker_id or "").strip()) < 6:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Enter a valid DigiLocker ID for demo")
        ...
        worker = worker_result.data[0]
        stored_phone = normalize_phone(worker.get("phone") or worker.get("phone_number", ""))
        provided_phone = normalize_phone(request.phone)
        if provided_phone != stored_phone:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Phone number does not match this profile")

        rc = await get_redis()
        await rc.setex(f"wa_session:{stored_phone}", 8 * 3600, "active")
        ...
        return {
            ...
            "demo_notice": "DigiLocker verification is mocked for demo only. No DigiLocker data is stored.",
            "session_expires_in_seconds": 8 * 3600,
            "profile": {
                "worker_id": worker_id,
                "name": worker.get("name", "Unknown"),
                "phone": worker.get("phone") or worker.get("phone_number", ""),
        ...
        logger.error(f"[SHARE_TOKEN] Session login error: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Session login failed: {str(e)}")
