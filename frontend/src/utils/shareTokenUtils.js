...
export const generateShareToken = async (
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/v1/share-tokens/generate`,
      {
        method: "POST",
        headers: {
...
export const verifyShareToken = async (token) => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/v1/share-tokens/verify/${token}`,
    );
...
export const getSharedWorkerProfile = async (token) => {
  try {
    console.log(
      `[SHARE] Fetching profile for token: ${token.substring(0, 8)}...`,
    );

    const response = await fetch(
      `${API_BASE_URL}/api/v1/share-tokens/profile/${token}`,
    );
